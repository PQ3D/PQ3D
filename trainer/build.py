...
class BaseTrainer():
    def __init__(self, cfg):
        ...
        if cfg.model.name == 'Query3D' or cfg.model.name == 'Query3DMerge':
            ...
            if 'mv' in cfg.model.memories or 'sem' in cfg.model.memories:
                cfg.data.load_scan_options.load_mv_feats = True
            if 'voxel' in cfg.model.memories or 'sem' in cfg.model.memories:
                cfg.data.load_scan_options.load_voxel_feats = True
            ...
            if len(cfg.model.memories) <= 2:
                cfg.model.unified_encoder.args.memory_dropout=0 # do not dropout memory when using only one scene representation
